"""
OR-Tools adapter for Workforce Management Harness.
...
from ..domain import WFMData, ScheduleResult, OptimizationResult
...
class ORToolsAdapter(BaseAdapter):
    ...
    def _validate_dependencies(self):
        ...
        if not has_ortools:
            raise ImportError("OR-Tools is not installed. Install with: pip install ortools==9.8.3296")
        ...
        try:
            ...
            for a in range(num_agents):
                for d in range(days - max_consecutive):
                    window_vars = [x[(a, d + i, s)] for i in range(max_consecutive + 1) for s in range(shifts_per_day)]
    ...
    def schedule(self, data: List[WFMData], **kwargs) -> AdapterResult:
        ...
        try:
            ...
            for a in range(num_agents):
                for d in range(days - max_consecutive):
                    window = [x[(a, d + i, s)] for i in range(max_consecutive + 1) for s in range(shifts_per_day)]
            ...
            for a in range(num_agents):
                ...
                for d in range(days):
                    ...
                    days_off_expr = cp_model.LinearExpr.Sum([1 for d in range(days) if days_worked_vars[d] == 0])
            ...
            if preference_bonuses:
                ...
                agent_totals = [sum(x[(a, d, s)] for d in range(days) for s in range(shifts_per_day)) for a in range(num_agents)]
                ...
                for d in range(days):
                    for s in range(shifts_per_day):
                        if solver.Value(x[(a, d, s)]) == 1:
                            ...
                            agent_shifts.append({
                                'day': d,
                                'date': str(unique_dates[d]) if d < len(unique_dates) else f'day_{d}',
                                'shift': shift_name,
                                'start_hour': s * 8,
                                'end_hour': (s + 1) * 8
                            })
