"""
Configuration validation for Workforce Management Harness.
...
class WFMBaseConfig(BaseModel):
    ...
    name: Optional[str] = Field(default=None, description="Configuration name", example="production_config")
    ...
    def validate_version(cls, v):
        ...
        if not all(c.isalnum() or c in '.-_' for c in v):
            raise ValueError("Version contains invalid characters. Use alphanumeric, dots, hyphens, and underscores only")
        return v
class IntervalConfig(WFMBaseConfig):
    ...
    interval_unit: TimeInterval = Field(..., description="Unit of time interval", example=TimeInterval.MINUTE)
    business_start_hour: int = Field(default=9, ge=0, le=23, description="Business start hour (0-23)")
    business_end_hour: int = Field(default=17, ge=0, le=23, description="Business end hour (0-23)")
    working_days: List[int] = Field(default_factory=lambda: list(range(1, 6)), 
...
class SLAConfig(WFMBaseConfig):
    ...
    average_speed_of_answer: float = Field(..., gt=0, description="Average speed of answer in seconds", example=180)
    acceptable_service_level: float = Field(default=0.90, gt=0, le=1, description="Acceptable service level")
    minimum_service_level: float = Field(default=0.95, gt=0, le=1, description="Minimum service level")
    abandoned_rate_target: Optional[float] = Field(None, ge=0, le=1, description="Abandoned call rate target")
    service_level_measurements: List[str] = Field(default_factory=list, 
                                                 description="Service level measurement types")
    
    @field_validator('target', 'acceptable_service_level', 'minimum_service_level', 'abandoned_rate_target')
    def validate_sla_values(cls, v, info):
        ...
        if 'target' in info.data and field_name == 'minimum_service_level' and v < info.data['target']:
            raise ValueError("minimum_service_level must be >= target")
        if 'acceptable_service_level' in info.data and field_name == 'minimum_service_level' and v > info.data['acceptable_service_level']:
...
class ShrinkageConfig(WFMBaseConfig):
    ...
    factors: Dict[str, float] = Field(default_factory=dict, description="Shrinkage factors by category")
    unplanned_absence_rate: float = Field(default=0.05, ge=0, le=1, description="Unplanned absence rate")
    planned_absence_rate: float = Field(default=0.02, ge=0, le=1, description="Planned absence rate")
    training_time_percentage: float = Field(default=0.10, ge=0, le=1, description="Training time percentage")
    meeting_time_percentage: float = Field(default=0.05, ge=0, le=1, description="Meeting time percentage")
    break_time_percentage: float = Field(default=0.15, ge=0, le=1, description="Break time percentage")
...
class OccupancyConfig(WFMBaseConfig):
    ...
    calculation_method: str = Field(default="utilization", description="Occupancy calculation method")
    include_break_time: bool = Field(default=True, description="Include break time in calculation")
    include_meeting_time: bool = Field(default=False, description="Include meeting time in calculation")
    include_training_time: bool = Field(default=True, description="Include training time in calculation")
...
class SkillConfig(WFMBaseConfig):
    ...
    default_appointment_length: Optional[float] = Field(None, gt=0, description="Default appointment length in seconds")
    required_skills: List[str] = Field(default_factory=list, description="Required skills for this skill")
    optional_skills: List[str] = Field(default_factory=list, description="Optional skills for this skill")
...
class ChannelConfig(WFMBaseConfig):
    ...
    queue_limit: Optional[int] = Field(None, gt=0, description="Queue limit")
    skill_required: bool = Field(default=True, description="Whether channel requires specific skills")
class ForecastConfig(WFMBaseConfig):
    ...
    seasonality: Optional[int] = Field(None, gt=0, description="Seasonality pattern")
    ...
    data_sources: List[str] = Field(default_factory=list, description="Data sources for forecasting")
...
class StaffingConfig(WFMBaseConfig):
    ...
    service_level_config: Optional[SLAConfig] = Field(None, description="Service level configuration")
    interval_config: Optional[IntervalConfig] = Field(None, description="Interval configuration")
    shrinkage_config: Optional[ShrinkageConfig] = Field(None, description="Shrinkage configuration")
    occupancy_config: Optional[OccupancyConfig] = Field(None, description="Occupancy configuration")
    skills: List[SkillConfig] = Field(default_factory=list, description="Skills configuration")
    channels: List[ChannelConfig] = Field(default_factory=list, description="Channels configuration")
    historical_data_hours: int = Field(default=168, gt=0, description="Historical data hours to analyze")
    demand_forecast_multiplier: float = Field(default=1.0, gt=0, description="Demand forecast multiplier")
...
class SchedulingConfig(WFMBaseConfig):
    ...
    min_hours_per_agent: float = Field(default=0, ge=0, description="Minimum hours per agent per period")
    ...
    allowed_shift_patterns: List[Dict[str, Any]] = Field(default_factory=list, description="Allowed shift patterns")
    ...
    time_window: Dict[str, Any] = Field(default_factory=dict, description="Time window for scheduling")
...
class OptimizationConfig(WFMBaseConfig):
    ...
    constraint_weights: Dict[str, float] = Field(default_factory=dict, description="Constraint weights")
    ...
    population_size: int = Field(default=100, gt=0, description="Population size for evolutionary algorithms")
...
class ValidationConfig(WFMBaseConfig):
    ...
    backup_before_validation: bool = Field(default=True, description="Create backup before validation")
class WFMConfig(WFMBaseConfig):
    ...
    forecast_config: Optional[ForecastConfig] = Field(None, description="Forecasting configuration")
    staffing_config: Optional[StaffingConfig] = Field(None, description="Staffing configuration")
    scheduling_config: Optional[SchedulingConfig] = Field(None, description="Scheduling configuration")
    optimization_config: Optional[OptimizationConfig] = Field(None, description="Optimization configuration")
    validation_config: Optional[ValidationConfig] = Field(None, description="Validation configuration")
    skills: List[SkillConfig] = Field(default_factory=list, description="Skills configuration")
    channels: List[ChannelConfig] = Field(default_factory=list, description="Channels configuration")
    ...
    def validate_configs_exist(self):
        ...
        if self.operating_profile in [OperatingProfile.INBOUND, OperatingProfile.BLENDED, OperatingProfile.MULTI_SKILL]:
            if self.forecast_config is None:
                raise ValueError(f"forecast_config is required for operating profile: {self.operating_profile}")
        if self.operating_profile in [OperatingProfile.INBOUND, OperatingProfile.OUTBOUND, OperatingProfile.BLENDED, OperatingProfile.MULTI_SKILL]:
            if self.staffing_config is None:
                raise ValueError(f"staffing_config is required for operating profile: {self.operating_profile}")
        if self.operating_profile in [OperatingProfile.INBOUND, OperatingProfile.OUTBOUND, OperatingProfile.BLENDED, OperatingProfile.MULTI_SKILL]:
            if self.scheduling_config is None:
                raise ValueError(f"scheduling_config is required for operating profile: {self.operating_profile}")
        return self
    ...
    def validate_channels_exist(cls, v, info):
        ...
        if info.data and 'operating_profile' in info.data and info.data['operating_profile'] == OperatingProfile.MULTI_SKILL:
            if not v:
                raise ValueError("At least one channel is required for multi_skill operating profile")
        return v
    ...
    def get_required_components(self) -> List[str]:
        ...
        if self.operating_profile in [OperatingProfile.INBOUND, OperatingProfile.OUTBOUND, OperatingProfile.BLENDED]:
